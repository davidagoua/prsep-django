...
class SSP {
	...
	static function simple ( $request, $pg_details, $table, $primaryKey, $columns, $filtroAdd=NULL )
	{
		...
		$select = "SELECT  ".implode(", ", SSP::pluckas($columns))."
		 ...
		 // $monfichier = fopen(base_url().'test.txt', 'r+');
					// ftruncate($monfichier,0);
					// fputs($monfichier,'1799'.json_encode($request, JSON_NUMERIC_CHECK));
		//Debug   // ,  sum(superficie_total) OVER() as superficie_global
		...
		// die();
 		...
 		// $superficie_global = $data[0]['superficie_global'];
		if ($filtroAdd !== NULL){
